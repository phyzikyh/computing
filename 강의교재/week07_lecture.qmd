...
> 여러 값을 목적에 맞게 저장하고 조회·변경하려면 어떤 자료구조를 선택해야 할까?

**도달 기준**

- 리스트·튜플·딕셔너리의 구조와 변경 가능성을 비교한다.
- 필요한 값을 안전하게 조회·추가·수정·삭제한다.
- 문제의 데이터 관계에 맞는 자료구조를 선택해 처리 절차를 구현한다.

**학습 경로**  개념 이해 → [코딩 실습](week07_code.qmd) → [학습 점검](week07_quiz.qmd)
:::

## 왜 자료구조인가: 데이터 컬렉션의 필연성 {#sec-why-data-structures}

학생 $100$명의 성적을 처리하기 위해 `score1`, `score2`, ..., `score100`처럼 100개의 개별 변수를 선언하는 것은 비효율적이며 프로그램의 확장성을 파괴합니다.

[자료구조(Data Structure)]{.key}는 **여러 개의 연관된 데이터를 하나의 묶음으로 구조화하여 컴퓨터 메모리에 효율적으로 저장하고 관리하는 체계적인 방식**입니다.

```{mermaid}
%%| fig-cap: "**그림 7-1.** 파이썬의 주요 복합 자료구조"
flowchart TD
